...
uniform point_light_structure point_lights[NUMBER_OF_POINT_LIGHTS];
...
void main()
{
   ...
   for (int index = 0; index < NUMBER_OF_POINT_LIGHTS; index++)
   {
	...
	result += calculatePointLight(point_lights[index], vertex_normal_coordinate, vertex_fragment_coordinate, view_direction);
...
vec3 calculate_point_light(point_light_structure light, vec3 normal, vec3 fragment_position, vec3 view_direction)
{
	...
	//  Specular shading...
	vec3 reflect_direction = reflect(-light_direction, vertex_normal_coordinate);
	...
	//  Combine results...
	vec3 ambient  = light.ambient  *        vec3(texture(material.texture,  vertex_texture_coordinate));
	vec3 diffuse  = light.diffuse  * diff * vec3(texture(material.texture,  vertex_texture_coordinate));
	vec3 specular = spec * vec3(texture(material.texture, vertex_texture_coordinate));
	ambient      *= attenuation;
	diffuse      *= attenuation;
